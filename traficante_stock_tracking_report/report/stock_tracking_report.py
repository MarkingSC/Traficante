...
class stockTrackingReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, products):
        ...
        sheet.merge_range('C3:H3', 'REPORTE DE TRAZABILIAD DEL ' + str(report_date_from.strftime('%d/%m/%Y')) + ' al ' + str(report_date_to.strftime('%d/%m/%Y')), company_format)
        sheet.write(4,0, 'Fecha', header_col_format)
        sheet.write(4,1, 'Producto', header_col_format)
        sheet.write(4,2, 'Costo', header_col_format)
        sheet.write(4,3, 'Lote', header_col_format)
        sheet.write(4,4, 'Ingreso del lote', header_col_format)
        sheet.write(4,5, 'Días inventario', header_col_format)
        sheet.write(4,6, 'Unidad de medida', header_col_format)
        sheet.write(4,7, 'Tipo de movimiento', header_col_format)
        sheet.write(4,8, 'Referencia', header_col_format)
        sheet.write(4,9, 'Doc. de origen', header_col_format)
        sheet.write(4,10, 'Usuario', header_col_format)
        sheet.write(4,11, 'Entradas', header_col_format)
        sheet.write(4,12, 'Salidas', header_col_format)
        sheet.write(4,13, 'Disponibles', header_col_format)
        ...
        for product in products:
            ...
            for line in move_lines.filtered(lambda line: line.product_id == product):
                ...
                sheet.write(row, 2, str(line.product_uom_id.name))
                sheet.write(row, 3, str(line.lot_product_cost))
                sheet.write(row, 4, str(line.lot_id.name))
                sheet.write(row, 5, str(line.lot_incoming_date.strftime('%d/%m/%Y') if line.lot_incoming_date else ''))
                sheet.write(row, 6, str(line.lot_iddle_time))
                sheet.write(row, 7, str(line.lot_picking_type.name if line.lot_picking_type else ''))
                sheet.write(row, 8, str(line.reference))
                sheet.write(row, 9, str(line.lot_origin_doc if line.lot_origin_doc else ''))
                sheet.write(row, 10, str(line.lot_uid.name if line.lot_uid.name else ''))
                sheet.write(row, 11, str(line.lot_incoming_qtty))
                sheet.write(row, 12, str(line.lot_outgoing_qtty))
                sheet.write(row, 13, str(line.lot_current_stock))
                row += 1
            ...
            sheet.write(product_row, 2, str(product.uom_id.name), header_col_format_gray)
            sheet.write(product_row, 3, '', header_col_format_gray)
            ...
            sheet.write(product_row, 9, '', header_col_format_gray)
            sheet.write(product_row, 10, '', header_col_format_gray)
            sheet.write(product_row, 11, str(product_lot_incoming_qtty), header_col_format_gray)
            sheet.write(product_row, 12, str(product_lot_outgoing_qtty), header_col_format_gray)
