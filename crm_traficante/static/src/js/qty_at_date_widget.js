odoo.define('sale_stock.QtyAtDateWidget', function (require) {
    ...
    var QtyAtDateWidget = Widget.extend({
        ...
        _updateData: function() {
            ...
            // Modificaciones para validar en función de la cantidad realmente disponible
            if (this.data.scheduled_date) {
                // Siempre se toma free_qty_today porque es la cantidad realmente disponible
                var qty_considered = this.data.free_qty_today;
                // Se determina si el pedido podrá ser abastecido
                //this.data.will_be_fulfilled = utils.round_decimals(qty_considered, this.fields.virtual_available_at_date.digits[1]) >= utils.round_decimals(this.data.qty_to_deliver, this.fields.qty_to_deliver.digits[1]);
                this.data.will_be_fulfilled = qty_considered >= this.data.qty_to_deliver;
                console.log('this.data.free_qty_today: ' + this.data.free_qty_today);
                console.log('this.data.qty_to_deliver: ' + this.data.qty_to_deliver);
                console.log('this.data.will_be_fulfilled : ' + this.data.will_be_fulfilled );
