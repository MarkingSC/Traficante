...
class AgeingView(models.TransientModel):
    ...
    def _get_partner_move_lines(self, data, partners, date_from, target_move,
        ...
        # El arreglo de periodos es [ 30+, 1-7, 8-14, 15-21, 22-30]
        history = []
        for i in range(5):
            ...
            if periods[str(i)]['start'] and periods[str(i)]['stop'] and i>1:
            ...
            elif periods[str(i)]['start'] and periods[str(i)]['stop'] and i==1:
                _logger.debug("**** EL RANGO TIENE AMBOS LIMITES Y ES 3 ")
                dates_query += " BETWEEN %s::date - 2 * INTERVAL '1 day' AND %s)"
            ...
            # iba a ser 28, pero tiene que ser 30
                ...
                dates_query += " <= %s::date - 2 * INTERVAL '1 day')"
