...
class SalesDetailedReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, goals):
        ...
        sheet.write(7,7, 'IEPS', header_col_format)
        sheet.write(7,8, 'IEPS %', header_col_format)
        sheet.write(7,9, 'IEPS $', header_col_format)
        sheet.write(7,10, 'IVA', header_col_format)
        sheet.write(7,11, 'PRECIO FINAL', header_col_format)
        sheet.write(7,12, 'RAZÓN SOCIAL', header_col_format)
        sheet.write(7,13, 'SUCURSAL / NOMBRE COMERCIAL', header_col_format)
        sheet.write(7,14, 'VENDEDOR', header_col_format)
        sheet.write(7,15, 'ESTADO DEL DOCUMENTO', header_col_format)
        sheet.write(7,16, 'ESTADO DEL CFDI', header_col_format)
        ...
        sheet.write(8,9, str("${:,.2f}".format(total_ieps_amount)), total_row_format)
        sheet.write(8,10, str("${:,.2f}".format(total_iva_amount)), total_row_format)
        sheet.write(8,11, str("${:,.2f}".format(total_price_total)), total_row_format)
    ...
    def print_line_row(self, row, sheet, line):
        ...
        sheet.write(row, 7, str(ieps_taxes_str))
        sheet.write(row, 8, str("{:,.2f}%".format(line.ieps_percent*100)))
        sheet.write(row, 9, str("${:,.2f}".format(line.ieps_amount)))
        sheet.write(row, 10, str("${:,.2f}".format(line.iva_amount)))
        sheet.write(row, 11, str("${:,.2f}".format(line.price_total)))
        sheet.write(row, 12, str(line.business_name))
        sheet.write(row, 13, str(line.commercial_name))
        sheet.write(row, 14, str(line.salesperson))
        sheet.write(row, 15, str(estado_doc))
        sheet.write(row, 16, str(estado_cfdi))
