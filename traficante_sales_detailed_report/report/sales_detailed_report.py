...
class SalesDetailedReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, goals):
        ...
        sheet.write(7,1, 'FOLIO FISCAL', header_col_format)
        sheet.write(7,2, 'FECHA', header_col_format)
        sheet.write(7,3, 'CODIGO', header_col_format)
        sheet.write(7,4, 'DESCRIPCIÓN', header_col_format)
        sheet.write(7,5, 'CANTIDAD', header_col_format)
        sheet.write(7,6, 'PRECIO', header_col_format)
        sheet.write(7,7, 'SUBTOTAL', header_col_format)
        sheet.write(7,8, 'DESCUENTO', header_col_format)
        sheet.write(7,9, 'IEPS', header_col_format)
        sheet.write(7,10, 'IEPS %', header_col_format)
        sheet.write(7,11, 'IEPS $', header_col_format)
        sheet.write(7,12, 'IVA', header_col_format)
        sheet.write(7,13, 'PRECIO FINAL', header_col_format)
        sheet.write(7,14, 'RAZÓN SOCIAL', header_col_format)
        sheet.write(7,15, 'SUCURSAL / NOMBRE COMERCIAL', header_col_format)
        sheet.write(7,16, 'VENDEDOR', header_col_format)
        sheet.write(7,17, 'RECEPTOR', header_col_format)
        sheet.write(7,18, 'ESTADO DEL DOCUMENTO', header_col_format)
        sheet.write(7,19, 'ESTADO DEL CFDI', header_col_format)
        ...
        sheet.merge_range('A9:E9', 'TOTAL', total_row_format)
        sheet.write(8,5, str("{:,.2f}".format(total_quantity)), total_row_format)
        sheet.write(8,6, str("${:,.2f}".format(total_price_unit)), total_row_format)
        sheet.write(8,7, str("${:,.2f}".format(total_price_subtotal)), total_row_format)
        sheet.write(8,8, '-', total_row_format)
        sheet.write(8,9, '-', total_row_format)
        sheet.write(8,10, '-', total_row_format)
        sheet.write(8,11, str("${:,.2f}".format(total_ieps_amount)), total_row_format)
        sheet.write(8,12, str("${:,.2f}".format(total_iva_amount)), total_row_format)
        sheet.write(8,13, str("${:,.2f}".format(total_price_total)), total_row_format)
        sheet.merge_range('N9:T9', '', total_row_format)
    ...
    def print_line_row(self, row, sheet, line):
        ...
        sheet.write(row, 1, str(line.move_id.folio_fiscal))
        sheet.write(row, 2, str(line.date))
        sheet.write(row, 3, str(line.product_code))
        sheet.write(row, 4, str(line.product_name))
        sheet.write(row, 5, str(line.quantity))
        sheet.write(row, 6, str("${:,.2f}".format(line.price_unit)))
        sheet.write(row, 7, str("${:,.2f}".format(line.price_subtotal)))
        sheet.write(row, 8, str("{:,.2f}%".format(line.discount)))
        sheet.write(row, 9, str(ieps_taxes_str))
        sheet.write(row, 10, str("{:,.2f}%".format(line.ieps_percent*100)))
        sheet.write(row, 11, str("${:,.2f}".format(line.ieps_amount)))
        sheet.write(row, 12, str("${:,.2f}".format(line.iva_amount)))
        sheet.write(row, 13, str("${:,.2f}".format(line.price_total)))
        sheet.write(row, 14, str(line.business_name))
        sheet.write(row, 15, str(line.commercial_name))
        #sheet.write(row, 15, str(line.salesperson))
        #sheet.write(row, 16, str(line.partner_id.executive_tracking))
        sheet.write(row, 16, str(line.partner_id.user_id.partner_id.name))
        sheet.write(row, 17, str(line.partner_id.vat))
        sheet.write(row, 18, str(estado_doc))
        sheet.write(row, 19, str(estado_cfdi))
